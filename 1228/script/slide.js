...
const slide_img = document.querySelectorAll('.slide li'); //(All은 복수일때) 상품목록
...
const m = 60; // 여백 너비
const s_count = 3; // 한 페이지에 보여질 상품 개수

let count = 0; // 이전, 다음 클릭 시 사용할 변수값 설정

slide.computedStyleMap.width = (img_w+m)*img_n-m+'px';
...
stop_btn.addEventListener('click', function(){
    clearInterval(Timer); // 시간객체를 없앰 (멈추게 할 수 있음)
});
